...
def run_optimization():
    ...
    print("    - Pada tahap ini, Bayesian Optimization (Optuna) mencari kombinasi hyperparameter terbaik.")
    print("    - Ruang Pencarian (Search Space) mencakup: Window Size, Filters, Units, Dropout, Learning Rate, & Batch Size.")
    print("    - Total Trial: 50 Kali percobaan untuk masing-masing model (BiLSTM & CNN-LSTM).")
    print("    - Algoritma Optuna mempelajari performa trial sebelumnya untuk menyempurnakan tebakan berikutnya.")
    ...
    best_trial_bi = study_bi.best_trial
    print(f"    Selesai. Best MAPE: {study_bi.best_value*100:.4f}% ditemukan pada Trial ke-{best_trial_bi.number + 1} (indeks trial: {best_trial_bi.number}).")
    ...
    print(f"    Selesai. Best MAPE: {study_cl.best_value*100:.4f}% ditemukan pada Trial ke-{best_trial_cl.number + 1} (indeks trial: {best_trial_cl.number}).")
    ...
    # Tambahkan metadata trial terbaik agar terbawa saat dimuat di tahap selanjutnya
    best_params['bilstm']['best_trial'] = best_trial_bi.number + 1
    ...
    df_best = pd.DataFrame([
        {'Model': 'BiLSTM + BO', 'Best_Trial': best_trial_bi.number + 1, 'Best_MAPE': f"{study_bi.best_value*100:.4f}%", **study_bi.best_params},
