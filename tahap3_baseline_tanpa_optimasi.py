...
def train_baseline():
    """
    Fungsi untuk melatih model standar (CNN saja, LSTM saja, CNN-LSTM tanpa optimasi)
    sebagai benchmark/pembanding awal di Bab IV.
    """
    print("\n" + "="*70)
    print(" [TAHAP 3: PELATIHAN MODEL BASELINE (TANPA OPTIMASI)] ")
    print(" [FASE KDD: DATA MINING - BASELINE MODELLING] ")
    print("="*70)
    
    print("\n>>> PENJELASAN METODOLOGI (UNTUK BAB IV):")
    print("    - Ketiga model baseline (CNN Saja, LSTM Saja, dan CNN-LSTM) dilatih menggunakan hyperparameter standar (default).")
    print("    - Konfigurasi Default: Epochs: 30, Batch Size: 32, Learning Rate: 0.001, Window Size: 30, Filters/Units: 64.")
    print("    - Model-model baseline ini TIDAK MENDAPATKAN HYPERPARAMETER OPTIMAL karena dilatih secara instan")
    print("      tanpa melalui proses pencarian cerdas ataupun penyelarasan parameter (tuning).")
    print("    - Performa baseline ini sangat krusial sebagai pembanding untuk membuktikan keunggulan optimasi Bayesian.")
    ...
    results = []
    # Parameter default untuk baseline tanpa optimasi
    window_size = 30
    batch_size = 32
    epochs = 30
    lr = 0.001
    ...
    # 2. Model 1: CNN Saja
    print("\n>>> [1/3] Melatih Model CNN Saja (Tanpa Optimasi)...")
    f, k = 64, 3
    m_cnn = Sequential([
    ...
    # 3. Model 2: LSTM Saja
    print("\n>>> [2/3] Melatih Model LSTM Saja (Tanpa Optimasi)...")
    u = 64
    m_lstm = Sequential([
        Input(shape=(window_size, X_train.shape[2])),
        LSTM(u),
    ...
    results.append({
        'Model': 'LSTM Saja (Tanpa Optimasi)',
    ...
    # 4. Model 3: CNN-LSTM Tanpa Optimasi
    print("\n>>> [3/3] Melatih Model CNN-LSTM Tanpa Optimasi...")
    f, k, u = 64, 3, 64
    m_hybrid = Sequential([
        Input(shape=(window_size, X_train.shape[2])),
        Conv1D(f, k, activation='relu'),
    ...
    print(">>> Model CNN-LSTM Tanpa Optimasi berhasil disimpan ke model_tanpa_optimasi.h5")
    y_pred_tanpa_optimasi = m_hybrid.predict(X_test, verbose=0)
    res_hybrid = evaluate_metrics(scaler_y.inverse_transform(y_test.reshape(-1,1)), 
                                  scaler_y.inverse_transform(y_pred_tanpa_optimasi))
    results.append({
        'Model': 'CNN-LSTM (Tanpa Optimasi)', 
    ...
    print("\n" + "="*100)
    print("TABEL REKAPITULASI EVALUASI BASELINE MODEL (BELUM OPTIMAL - UNTUK DATA TESTING BAB IV)")
    print("="*100)
